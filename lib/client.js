 ...
 *
 * 「官方条显示不到」的判定必须与官方条同源:官方 QueueDock 读的是宿主 inbox 投影
 * (useProjection('inbox')['next-turn']),插件也用同一个来源做差集。历史教训:曾经读
 * useSession(s => s.queue) —— 核心删掉该字段后它恒为 undefined,差集恒空,于是插件
 * 把官方条已有的行又画了一遍(输入框上方出现两条一样的排队消息)。
 */
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function QueueNotice(props) {
      ...
      // 官方队列条渲染的行 = 宿主 inbox 投影的 next-turn(ui-conversation 的 QueueDock 读的就是
      // 它)。**不能读 SessionSnapshot**:核心 72f2e71070 已删掉 `snapshot.queue`,那里永远是
      // undefined ⇒ 差集恒为空 ⇒ 插件把官方条已有的行再画一遍(重叠条)。本插件只补官方条显示
      // 不到的行,官方那条才是权威。
      // 返回拼接串而不是数组,避免每次快照都换引用而反复重渲染。
      var useProjection = props.useProjection
      var officialIds = useProjection === undefined
        ? ''
        : useProjection('inbox', function (inbox) {
          var rows = (inbox && inbox['next-turn']) || []
          return rows.map(function (row) { return row.id }).join(',')
        }) || ''
      // 队列被消费/开始新轮次时也要重读:继承项本来就不在官方投影里(那正是本插件存在的
      // 理由),它被认领时 officialIds 不会变,只有 running 会变。
      var running = props.useSession === undefined ? false : props.useSession(function (snapshot) { return snapshot.running })
      ...
      }, [sessionId, officialIds, running])

      if (!sessionId || items === null) return null
      var official = officialIds === '' ? [] : officialIds.split(',')
      var missing = items.filter(function (item) { return official.indexOf(item.id) === -1 })
      if (missing.length === 0) return null
      var inheritedCount = missing.filter(function (it) { return it.inherited }).length
      ...
      var headerText = '⚠️ ' + missing.length + ' 条排队消息(官方队列条未显示)'
        + (inheritedCount === missing.length ? ' · 全部继承自原会话' : inheritedCount > 0 ? ' · 其中 ' + inheritedCount + ' 条继承自原会话' : '')
      var headerHint = '这些项在官方队列条上不会出现(通常是 fork 继承来的);发送时会先于你新发的消息送达模型。可用右侧按钮逐条编辑或删除。'
          ...
          React.createElement('div', { style: headerStyle },
            ...
            React.createElement('div', { style: actionsStyle },
              React.createElement('button', {
                type: 'button', style: actionStyle(busyId !== null), title: '清除以上全部', 'aria-label': '清除以上全部',
