...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function QueueNotice(props) {
      ...
      // 官方队列条渲染的那一条(客户端快照里 placement === 'queued' 的行)。
      // 本插件只补它显示不到的行:两边同时出现会重复,且官方那条才是权威。
      // 返回拼接串而不是数组,避免每次快照都换引用而反复重渲染。
      var useSession = props.useSession
      var officialIds = useSession === undefined
        ? ''
        : useSession(function (snapshot) {
          var rows = snapshot.queue || []
          return rows.filter(function (row) { return row.placement === 'queued' })
            .map(function (row) { return row.id }).join(',')
        }) || ''
