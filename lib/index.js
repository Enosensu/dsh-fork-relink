 ...
 * /log-prune/queue/remove、/log-prune/queue/edit(经官方 updateQueue 的 remove/edit),
 * 客户端在输入框上方的 dock 里按官方队列条的尺寸与交互显示这些排队项:
 * 逐条编辑(右侧按钮进入编辑态,Enter 保存 / Esc 取消)、逐条删除、全部清除。
 *
...
/**
 * 折叠 splice 序列为未消费的**排队轮次**(agent/inbox/spliced 的 splice 语义)。
 *
 * 只取 `next-turn`:那是官方队列条渲染的那一条(客户端里 `placement === 'queued'`),
 * 本插件补的就是它显示不到的行,语义必须与它一致,不能把 `next-step`(插话/上下文)
 * 混进来冒充"将先于新消息送达"。
 */
function foldInbox(events) {
  const queue = []
  for (const evt of events) {
    if (evt.type !== 'agent/inbox/spliced' || evt.data.target !== 'next-turn') continue
...
/** 队列项的可编辑文本:拼接全部 text 块,不截断(客户端只用 CSS 省略号做显示截断)。 */
...
export function apply(ctx) {
  ...
  // 队列项编辑:经官方 updateQueue edit(与官方队列条同一入口,同样要求会话处于打开状态)
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/log-prune/queue/edit',
    handler: async (req, res) => {
      try {
        if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method-not-allowed' }); return }
        if (!requestFromSameOrigin(req)) { sendJson(res, 403, { ok: false, error: 'forbidden' }); return }
        const body = await readJsonBody(req, 64 * 1024)
        const { sessionId, itemId, text } = body
        if (typeof sessionId !== 'string' || !/^[A-Za-z0-9-]+$/.test(sessionId) || sessionId.length > 128
          || typeof itemId !== 'string' || itemId.length === 0 || itemId.length > 128
        ...
        // 与官方同规则:编辑内容必须含非空白文本
        if (text.trim() === '') { sendJson(res, 400, { ok: false, error: 'empty-text' }); return }
        await ctx.sessionController.updateQueue({
          sessionId,
          itemId,
          action: { kind: 'edit', content: [{ type: 'text', text }] },
        })
        log('queue item edited', { sessionId, itemId, length: text.length })
        sendJson(res, 200, { ok: true })
      } catch (error) {
        log('queue edit failed', { message: String(error?.message ?? error) })
        sendJson(res, 400, { ok: false, error: 'edit-refused', message: String(error?.message ?? error) })
      }
    },
  }))
