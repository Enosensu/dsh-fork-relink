...
/** 读取一个会话的未消费队列项(活体优先;冷会话读日志文件)。 */
export async function readQueueItems(ctx, sessionId) {
  const live = ctx.sessions.get(sessionId)
  if (live !== undefined) {
    const events = live.session.snapshotEvents()
    const inheritedCut = live.session.inheritedEventCount
    return foldInbox(events).map(m => ({
      id: m.id,
      text: messageText(m),
      inherited: typeof m.seq === 'number' ? m.seq < inheritedCut : false,
    }))
  }
  const file = await findSessionLogFile(sessionId)
  if (file === undefined) return []
  const parsed = readLogLines(await readFile(file)).map(l => JSON.parse(l))
  const lastSeedMarker = parsed.map((e, i) => ({ e, i })).filter(x => x.e.type === 'session/end-seed').pop()
  const inheritedCut = lastSeedMarker === undefined ? 0 : parsed[lastSeedMarker.i].seq + 1
  return foldInbox(parsed).map(m => ({
    id: m.id,
    text: messageText(m),
    inherited: typeof m.seq === 'number' ? m.seq < inheritedCut : false,
  }))
