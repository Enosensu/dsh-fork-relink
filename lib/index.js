 ...
 */
import { copyFile, mkdir, readdir, readFile, rename, stat, appendFile } from 'node:fs/promises'
...
/** 定位一个会话的工作区目录(sessions 根下的 <工作区>/<sessionId>/)。 */
 ...
 */
async function relinkChildren(ctx, parentId, childId) {
  if (parentId === childId) return { relinked: [], skipped: [] }
  const workspaceDir = await findSessionWorkspace(parentId)
  if (workspaceDir === undefined) return { relinked: [], skipped: [] }
  const relinked = []
  const skipped = []
...
function relinkContained(ctx, parentId, childId) {
  void relinkChildren(ctx, parentId, childId)
    .then((result) => {
      if (result.relinked.length > 0 || result.skipped.length > 0) {
        log('relinked', { parentId, childId, ...result })
      }
      return result
    })
    .catch((error) => {
...
export function apply(ctx) {
  const disposer = ctx.on('session/created', (session) => {
    ...
    relinkContained(ctx, header.parentSession, session.id)
  })
  return disposer
}
