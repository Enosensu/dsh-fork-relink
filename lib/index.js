...
async function copyChildren(ctx, sourceId, newParentAgent) {
  ...
  const seedText = JSON.stringify(newParentAgent.session.snapshotEvents())
  for (const child of await findChildSessions(workspaceDir, sourceId)) {
    if (ctx.agents.get(child.id) !== undefined) { skipped.push(child.id); continue }
    if (!seedText.includes(child.id)) { unrelated.push(child.id); continue }
    const observed = await ctx.sessionQuery.observeSession(child.id)
    const descriptor = descriptorOf(observed.events)
    ...
    await ctx.agents.create({
      ...
      inheritedEventCount: observed.events.length,
